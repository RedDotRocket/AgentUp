...
def serve(config, host, port, reload):
    ...
    Examples:
        ...
        agentup agent serve --no-reload             # Disable auto-reload
    """
    # Import and call the run functionality
    from pathlib import Path

    from . import run

    return run.run.callback(Path(config), host, port, reload, False, False)
